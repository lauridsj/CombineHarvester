...
for i, sigmayt in enumerate([-2, -1, -0.5, 0, 0.5, 1, 2]):
    yt = yt_nom + sigmayt * dyt_up if sigmayt >= 0 else yt_nom + sigmayt * dyt_down
    n_combine = get_combine_events(sigmayt)
    n_naive = get_naive_events(yt)

    print("Agreement for g = {g}: {a}".format(g=yt, a=np.all(np.isclose(n_combine, n_naive))))
