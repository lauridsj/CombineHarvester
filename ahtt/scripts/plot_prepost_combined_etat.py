...
from drawings import etat_blurb, get_poi_values
...
def plot_eventperbin(ax, bins, centers, smhists, total, data, log, fit, channel):
    ...
    for ibin in range(len(bins) - 1):
        ...
        ax.fill_between(
            ...
            label = f"{fstage}fit uncertainty" if ibin == 0 else None,
            **hatchstyle)
    ax.errorbar(
        centers,
        data[0] / width,
    ...
    if log:
        ...
        ymax = 1.08 if "j" in channel else 1.16
...
def plot_ratio(ax, bins, centers, data, total, signals, gvalues, sigscale, fit, fitstep):
    ...
    labels = [" "*len(fittype), *labels_signals]
...
def plot(channel, year, fit,
    ...
    if plotupper:
        for j, (variable, edges) in enumerate(extra_axes.items()):
            for i in range(num_extrabins):
                ...
                ax1.text(1 / num_extrabins * (i + 0.5), 0.89 - j * 0.13, text, horizontalalignment = "center", fontsize = 19, transform = ax1.transAxes)
        ax1.minorticks_on()
    ...
    if fitstep == "p":
        ...
        fittype = r"Postfit (BG + $\mathbf{\eta_{\mathrm{t}}}$ + A/H)"
    elif fitstep == "sah":
...
for channel, year in product(channels, years):
    for fitstep in fits:
        ...
        f = feta if fitstep == "s" else fah
        ifile = args.ifile if fitstep == "s" else args.ifileah
        ...
        if fitstep == "s" or fitstep == "sah" or fitstep == "p":
