...
def plot_pull(oname, labels, isimpact, impactsb, pulls, nuisances, extra, point, reverse, transparent, plotformat):
    ...
    for ii, nn in enumerate(nuisances):
        ...
        if ii % nuisance_per_page == nuisance_per_page - 1 or ii == len(nuisances) - 1:
            ...
            print(imu[0][0:, ii - nuisance_per_page : ii + 1])
            rmax = max([[np.amax(imp[0:, ii - nuisance_per_page : ii + 1]) for imp in imu],
                        [np.amax(imp[0:, ii - nuisance_per_page : ii + 1]) for imp in imd]]) if isimpact else 1.5
