...
def ahtt_max_coupling(parity, mah, wah, relwidth = True):
    ...
    wah_rel = (wah if relwidth else wah / mah) * 100
    
    wdata = np.loadtxt(args.csv, delimiter=",", skiprows=1)

    len_caPhi = 199
    len_ma = 200
    
    wdata_caPhi = wdata[:,6].reshape((len_caPhi, len_ma))
    wdata_ma = wdata[:,8].reshape((len_caPhi, len_ma))
    wdata_relwidth = wdata[:,9].reshape((len_caPhi, len_ma))
    
    caPhi_limit = np.empty(len_ma)
    for i in range(len_ma):
        #minind = np.argmin(abs(wdata_relwidth[:,i] - args.width))
        tck = sciint.splrep(wdata_caPhi[:,i], wdata_relwidth[:,i] - wah_rel, s=0)
        roots = sciint.sproot(tck)
        if not len(roots) == 1:
            breakpoint()
        caPhi_limit[i] = roots[0]

    return sciint.interp1d(wdata_ma[0,:], caPhi_limit, fill_value="extrapolate", kind="cubic")(mah)
...
def draw_1D(onames, limits, labels, xaxis, yaxis, ltitle, gcurve, interpolate, drawband, observed, formal, cmsapp, luminosity, a343bkg, transparent):
    ...
    if not hasattr(draw_1D, "colors"):
        draw_1D.colors = OrderedDict([
            ...
            (2    , [{"exp2": "#f5bb54", "exp1": "#607641", "exp0": "0", "expl": "dashed", "obsf": "#ffffff", "obsl": "#0033a0", "alpe": 1., "alpo": 0.},
                     {"exp2": "#6699ff", "exp1": "#3366ff", "exp0": "#0033cc", "expl": (0, (1, 1)), "obsf": "#ffffff", "obsl": "tab:red", "alpe": 0.4, "alpo": 0.}]),
    ...
    for ii, yy in enumerate(yvalues):
        if drawband or ii == 0:
            ax.fill_between(xvalues, np.array(yy["exp-2"])*alp_factor, np.array(yy["exp+2"])*alp_factor,
    ...
    for ii, yy in enumerate(yvalues):
        if drawband or ii == 0:
            ax.fill_between(xvalues, np.array(yy["exp-1"])*alp_factor, np.array(yy["exp+1"])*alp_factor,
    ...
    for ii, yy in enumerate(yvalues):
        if drawband or ii == 0:
            ax.plot(xvalues, np.array(yy["exp0"])*alp_factor, color = draw_1D.colors[len(limits)][ii]["exp0"], linestyle = draw_1D.colors[len(limits)][ii]["expl"], linewidth = 1.5)
    ...
    if '_m' in onames[0] or '_w' in onames[0]:
        ...
        may_partial_g = [gg + 0.2 for gg in max_partial_g]
    ...
    if observed:
        for i1, yy in enumerate(yvalues):
            ...
            cf = ax.contourf(xv, yv*alp_factor, zv, [-1., 0.05], colors = cols, alpha = draw_1D.colors[len(limits)][i1]["alpo"])
            ...
            label = "Observed" if labels[i1] == "" else "Obs. " + labels[i1]
            handles.append((mln.Line2D([0], [0], color = mcl.to_rgba(draw_1D.colors[len(limits)][i1]["obsl"], 1.), linestyle = "solid", linewidth = 1.5), label))

    ymin *= alp_factor
    ymax1 *= alp_factor * 0.8
    ...
    plt.ylabel("$|c_\\mathrm{t}| / f_\\mathrm{a} \\, [\\mathrm{TeV}^{-1}]$", fontsize = 26, loc = "top")
    ax.margins(x = 0, y = 0)
    ...
    legend = ax.legend(first(handles), second(handles),
	               loc = "upper right", ncol = 2 if len(limits) < 3 else 3, bbox_to_anchor = (lmargin, 1. - lheight, lwidth, lheight - 0.012),
    ...
    if ymax2 > 1.75:
        ax.yaxis.set_major_locator(mtc.MultipleLocator(1.0))
    ax.minorticks_on()
...
def draw_variable(var1, onames, points, directories, otags, labels, yaxis, interpolate, onepoi, drawband, observed, formal, cmsapp, luminosity, a343bkg, transparent, dump_spline):
    ...
    for vv in var2s:
        ...
        axislabel = 'a' if var1 == "mass" else ('a', 'a')
        legendtext = ('a', vv, 'a') if var1 == "mass" else ('a', vv)
        draw_1D([oname.format(www = 'w' + str(vv).replace('.', 'p') if var1 == "mass" else 'm' + str(int(vv))) for oname in onames],
                ...
                r'$\Gamma_{\mathrm{\mathsf{%s}}}^{\mathrm{\mathsf{pred}}} \,>\, \Gamma_{\mathrm{\mathsf{%s}}}$' % ('a', 'a'),
                parse_point_quantile(interpolate), drawband, observed, formal, cmsapp, luminosity, a343bkg, transparent)
    ...
    if args.function == "natural":
        if len(apnt) > 0:
            draw_natural(["{ooo}/A_limit_natural_{mod}{tag}{f}".format(ooo = args.odir, mod = "one-poi" if args.onepoi else "g-scan", tag = args.ptag, f = f) for f in args.fmt],
                         apnt, adir, otags, args.label, axes["mass"] % 'a', axes["ttcoupling"] % apnt[0][0], args.interpolate, args.onepoi, args.drawband, args.observed, args.formal, args.cmsapp, args.luminosity, args.a343bkg, args.transparent)
        if len(hpnt) > 0:
            draw_natural(["{ooo}/H_limit_natural_{mod}{tag}{f}".format(ooo = args.odir, mod = "one-poi" if args.onepoi else "g-scan", tag = args.ptag, f = f) for f in args.fmt],
                         hpnt, hdir, otags, args.label, axes["mass"] % 'a', axes["ttcoupling"] % hpnt[0][0], args.interpolate, args.onepoi, args.drawband, args.observed, args.formal, args.cmsapp, args.luminosity, args.a343bkg, args.transparent)
    else:
        if len(apnt) > 0:
            draw_variable(args.function,
                          ...
                          apnt, adir, otags, args.label, axes["ttcoupling"] % 'a', args.interpolate, args.onepoi, args.drawband, args.observed, args.formal, args.cmsapp, args.luminosity, args.a343bkg, args.transparent, args.dump_spline)
        if len(hpnt) > 0:
            draw_variable(args.function,
