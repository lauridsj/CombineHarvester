...
def get_interval(parameter, best_fit, fits, delta = 1., epsilon = 1.e-2):
    ...
    if None not in uncertainties and abs(uncertainties[0] - uncertainties[1]) < epsilon:
        return f", ${parameter} = {math.ceil(math.log10(best_fit[0]))} \pm {round(uncertainties[0], -math.ceil(math.log10(epsilon)))}$"
    else:
        result = f", ${parameter} = {math.ceil(math.log10(best_fit[0]))}"
